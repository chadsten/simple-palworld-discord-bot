 ...
 *    window that poll happened to land.
 *
 ...
 * restart interval. Without it, a persistently failing restart re-arms on EVERY
 * monitor poll - uptime is already past the interval, so secondsRemaining is
 * negative and the countdown collapses to an immediate retry, forever.
 */
 ...
 *
 * Warnings whose fire time has already passed are SKIPPED, never fired late - a
 * poll landing 4 minutes out schedules 3/2/1 only, instead of bursting four
 * stale "restarting in 30 minutes!" lines into chat at once.
 *
...
export function armRestartCountdown(uptimeSeconds, performRestart) {
  ...
  const secondsRemaining = config.autoRestart.intervalHours * 3600 - uptimeSeconds;
  if (secondsRemaining > ARM_THRESHOLD_SECONDS) {
    logger.debug(`Not in restart window yet (${Math.round(secondsRemaining / 60)} minutes remaining)`);
    return false;
  }
  ...
  for (const minutes of WARNING_MINUTES) {
    ...
    if (delaySeconds <= 0) continue; // already past - skip rather than fire late
    const unit = minutes === 1 ? 'minute' : 'minutes';
    addTimer(delaySeconds * 1000, () => announce(`Server restarting in ${minutes} ${unit}!`));
    scheduled.push(minutes);
  }

  // A negative remainder (uptime already past the interval, e.g. the feature was
  // enabled mid-session) collapses to an immediate restart with no warnings.
  addTimer(Math.max(secondsRemaining, 0) * 1000, async () => {
  ...
  logger.info(
    `Restart countdown armed: restarting in ${eta}; ` +
